...
const protectRoute = async (req: Request, res: Response, next: NextFunction) : Promise<any> => {
    try{
        ...
        if(!token){
            return res.status(401).json({error: "Unaotorized - no token provided"})
        }
        const decoded = jwt.verify(token, process.env.JWT_SECRET!) as DecodedToken;

        if(!decoded) {
            return res.status(401).json({error: "Unaotorizde - Invalid Token"});
        }
        ...
        next();//pozovi sledecu funkciju getMe?
    }catch(error: any){
